...
describe('UniswapV2Pair', () => {
  ...
  beforeEach(async () => {
    ...
    factory = await contractFactory.deploy(wallet.address, overrides)
  ...
  it('price{0,1}CumulativeLast', async () => {
    ...
    await mineBlock(network.provider, blockTimestamp + 1)
    await pair.sync(overrides)
    ...
    await token0.transfer(pair.address, swapAmount)
    await mineBlock(network.provider, blockTimestamp + 10)
    ...
    await mineBlock(network.provider, blockTimestamp + 20)
    await pair.sync(overrides)
